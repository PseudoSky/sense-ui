...
/**
 * @ngdoc service
 * @name senseUiApp.Timeseries
 * @description
 * # Timeseries
 * Factory in the senseUiApp.
 */
angular.module('senseUiApp')
  .factory('Timeseries', function () {
    // Service logic
    // ...
      function Timeseries(data, options) {
        ...
        */
        ...
        // }, options);
        if(!options){
          options={keys:{date:'timestamp',value:'value'}};
        }
        if(_.isArray(data) && _.isPlainObject(data[0]) && options.keys){
          this.data=Timeseries.adapter.fromDB(data, options.keys);
          // _.zip(_.pluck(data,options.keys[0]),_.pluck(data,options.keys[1]))
        }else{
          this.data     = data;
        }
        this.data=Timeseries.adapter.fromDB(data, options.keys);
